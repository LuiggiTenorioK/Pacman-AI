    ...
    def registerInitialState(self, state):
        ...
        while(currentState.getFood().count() > 0):
            ...
            for action in nextPathSegment:
                ...
                if action not in legal:
                    t = (str(action), str(currentState))
                    raise Exception, 'findPathToClosestCorner returned an illegal move: %s!\n%s' % t
                currentState = currentState.generateSuccessor(0, action)
        self.actionIndex = 0
        print 'Path found with cost %d.' % len(self.actions)
...
def closestPoint(fromPoint, candidatesList,distanceFunction):
    if len(candidatesList) == 0:
        return None

    closestCorner = candidatesList[0]
    closestCost = distanceFunction(fromPoint, closestCorner)
    for candidate in candidatesList[1:]:
        thisCost = distanceFunction(fromPoint, candidate)
