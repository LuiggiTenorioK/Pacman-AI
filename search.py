        ...
        for successor in problem.getSuccessors(node):
            ...
            if (successor[0] not in explored) and (successor not in [x for x,_ in frontier.list]):
                if problem.isGoalState(successor[0]):
                    print('Search nodes in frontier: %d' % len(frontier.list))
                    return path
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    while not frontier.isEmpty():
        ...
        if problem.isGoalState(s):
            print('Search nodes in frontier: %d' % len(frontier.heap)) #[x[1] for x in path][1:]
                ...
                if (successor[0] in [x for x,_ in frontier_fin.list]):
                    for node,action in frontier_fin.list:
                        if (node == successor[0]):
                            ...
                            route.reverse()
                            print('Search nodes in frontier: %d' % len(frontier_ini.list + frontier_fin.list)) #path + route
                            return path + route
                ...
                if (successor[0] in [x for x,_ in frontier_ini.list]):
                    for node,action in frontier_ini.list:
                        if (node == successor[0]):
                            ...
                            route.reverse()
                            print('Search nodes in frontier: %d' % len(frontier_ini.list + frontier_fin.list)) #action + route
                            return action + route
